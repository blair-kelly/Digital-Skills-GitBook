...
[Multifactor authentication](https://www.cyber.gov.au/mfa) is a feature available in many password managers which adds an extra layer of security to your passwords. By requiring identification not only via the main password but also by using an additional factor (e.g. code delivered via SMS or email, response via a physical token, etc.) it becomes even harder for someone to gain access.
